    ...
    case "${CMD}" in
      "test")
        if [[ -n $(find . -name "Godeps.json") ]]; then
          ...
          go test -race $(go list ./... | grep -v /vendor/ | grep -v vertical-pod-autoscaler/e2e | grep -v cluster-autoscaler/apis | grep -v cluster-autoscaler/integration | grep -v cluster-autoscaler/cloudprovider)
          if [[ $project_name == cluster-autoscaler ]];then
